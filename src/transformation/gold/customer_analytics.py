...
from src.common.constant import (
    ...
    MINIO_SILVER_PRODUCTS_PATH, MINIO_SILVER_CARTS_PATH
)
...
def build_customer_analytics(spark):
    ...
    customer_analytics_df = (
        ...
        .join(
            users_df.alias("u"),
            F.col("c.carts_user_id") == F.col("u.id"),
            "left"
        )
        .join(
            products_df.alias("p"),
        ...
        .withColumn(
            "discount_amount",
            (
                    F.col("cart_items_total")
                    - F.col("cart_items_discounted_total")
            ).cast("decimal(12,2)")
        )
        .withColumn(
            "gross_revenue",
    ...
    customer_spend_df = (
        ...
        .withColumn(
            "customer_segment",
            F.when(
                F.col("customer_total_spend") >= 50000,
                "VIP"
            )
            .when(
                F.col("customer_total_spend") >= 10000,
                "Premium"
            )
            .otherwise("Regular")
        )
    ...
    gold_df = (
        ...
        .join(
            ...
            F.col("a.carts_user_id")
            ==
            F.col("s.carts_user_id"),
            "left"
        )
        .select(
    ...
    if record_count == 0:
        raise ValueError(
            "Gold dataset contains no records"
        )

    logger.info(f"Gold dataset record count: {record_count}")
    logger.info(f"Gold Dataframe is created and validated")
